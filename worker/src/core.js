...
For local search or investigate actions, narration-only is common. When fictionally justified, reveal at most one small local clue, one modest local item, or one plausible nearby passage. Use spawn_item only for a pickup-able ground item, create_prop for a persistent visible clue that is not inventory, create_transition only for a plausible nearby passage discovered by search, and add_memory or set_flag for lasting knowledge. Do not create valuable or arbitrary rewards, major architecture, distant locations, or auto-pickup into inventory unless the player explicitly takes or receives the item. If narration says the search discovered a concrete persistent passage, item, or clue prop, include the matching allowed effect. If no effect is emitted, narration must not imply a new clickable or physical world object now exists. If a passage destination is uncertain or unsupported by context, prefer a clue prop or narration-only.
...
For using a selected inventory item with an NPC, narration-only NPC response is common. You may use add_memory, set_flag, or safe set_entity_state on the resolved NPC target. Use remove_item or consume_item only when the action clearly gives, hands over, or consumes the exact selected inventory item; never remove a different item. Do not use give_item, spawn_item, terrain, prop, movement, damage, or unrelated NPC mutation for this slice.
