...
test('valid request calls Workers AI once with default model and preserves the trust boundary', async () => {
  ...
  assert.match(input.messages[1].content, /The required actionId for your output is exactly:\naction-1/);
  assert.match(input.messages[1].content, /Untrusted game data/);
  assert.deepEqual(JSON.parse(input.messages[1].content.split('Untrusted game data (serialized gm_request_v1):\n')[1]), validRequest());
  assert.equal(input.response_format.type, 'json_schema');
  const schema = input.response_format.json_schema;
  assert.deepEqual(schema.required, ['protocol', 'actionId', 'narration', 'resolution', 'effects', 'memory']);
  assert.deepEqual(schema.properties.protocol.enum, ['gm_outcome_v1']);
  assert.deepEqual(schema.properties.resolution.properties.result.enum, ['success', 'failure', 'partial', 'blocked', 'uncertain']);
  assert.equal(schema.properties.effects.maxItems, 6);
  assert.equal(schema.properties.memory.maxItems, 6);
...
test('Worker preserves the model actionId for browser-side correlation', async () => {
  const mismatched = { ...outcome, actionId: 'model-returned-id' };
  const response = await handleRequest(browserRequest({ request: validRequest() }), makeEnv({ AI: makeAI({ response: mismatched }) }));
  assert.equal(response.status, 200);
  assert.equal((await response.json()).outcome.actionId, 'model-returned-id');
});
